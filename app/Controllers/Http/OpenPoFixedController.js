'use strict'
const OpenPoFixedMod    = use('App/Models/OpenPoFixed')
...
class OpenPoFixedController {

    async fetch_supplier_open({ response }) {
        let list_supplier_open = await OpenPoFixedMod.fetch_supplier_open()
    ...
    async fetch_temporary_item({ request, response }) {
        ...
        let check_posted = await OpenPoFixedMod.check_open_po_header_posted(p_invoice_no, p_supplier)
        if (check_posted != 0) {
            throw new CustomException({ message: "ANG INVOICE NUMBER NA ITO AY NAGAMIT NA SA GANITONG SUPPLIER"})
        }

        let po_header            = await OpenPoFixedMod.fetch_open_po_header(p_invoice_no, p_supplier)
        let list_temporary_items = []
        let temp_id = ""
        if (po_header != 0) {
                temp_id          = po_header.id
            list_temporary_items = await OpenPoFixedMod.fetch_temporary_item(temp_id)
        }
    ...
    async delete_items({request, response}) {
        ...
        let count_items = await  OpenPoFixedMod.counts_items(p_temp_id)
        if (count_items == 1 || p_type == 'cancel') {
            await OpenPoFixedMod.delete_header_items(p_temp_id, p_supplier)
        }

        let temp_id = (p_type == 'cancel') ? p_temp_id : null
        await OpenPoFixedMod.delete_items(p_id, temp_id)
    ...
    async add_item({ request, response}) {
        ...
        let open_po = await OpenPoFixedMod.fetch_open_po_header(inv_no, vendor_code)
        let temp_id

        if (open_po == 0) {
            temp_id = await OpenPoFixedMod.add_open_po_header(data, user_id)
        } else {
            temp_id = open_po.id
        }
        
        let check_open_items = await OpenPoFixedMod.check_open_items(data, temp_id)
        if (check_open_items > 0) {
            throw new CustomException({ message: `ITEM IS ALREADY EXIST` }, 401)
        }

        await OpenPoFixedMod.add_open_po_items(data, temp_id)
    ...
    async post_receiving({ request, response }) {
        ...
        let po_detail = await OpenPoFixedMod.fetch_current_open_po(user_id, p_temp_id, p_vendor_code)
        if (po_detail == 0) {
        ...
        let product_id_list =  await OpenPoFixedMod.fetch_open_details_product_id(temp_id)
        ...
        if (selling_area_negative.length == 0) {
           receiving_no = await OpenPoFixedMod.post_receiving(temp_id, p_inv_no, p_remarks, p_vendor_code, user_id)
        } else {
            receiving_no = await OpenPoFixedMod.post_receiving(temp_id, p_inv_no, p_remarks, p_vendor_code, user_id, selling_area_negative)
        }
...
module.exports = OpenPoFixedController
