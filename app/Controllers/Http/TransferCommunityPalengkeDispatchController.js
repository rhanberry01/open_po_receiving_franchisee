'use strict'
const TransferDispatchMod    = use('App/Models/TransferCommunityPalengkeDispatch')
...
class TransferCommunityPalengkeDispatchController {
    ...
    async fetch_barcode({ request, response }) {
        ...
        let location_to = transfer.br_code_in
    ...
    async post_receiving({ request, response }) {
        ...
        let checkPosted = await TransferDispatchMod.checkPosted(p_transfer_no)
        if(checkPosted) {
          throw new CustomException({ message: 'PL # is already posted '})
        }
...
module.exports = TransferCommunityPalengkeDispatchController
