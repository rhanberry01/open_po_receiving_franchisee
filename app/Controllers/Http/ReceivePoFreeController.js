'use strict'
const ReceivePoMod    = use('App/Models/ReceivePoFree')
...
class ReceivePoFreeController {
    ...
    async fetch_barcode({ request, response }) {
        ...
        let receive_qty =  ReceivePoMod.receive_qty(product_id, p_order_no, p_qty)
        let po__qty     =  ReceivePoMod.po_qty(product_id, p_order_no)

        let promise_qty_array = await Promise.all([receive_qty, po__qty])
            receive_qty = promise_qty_array[0]
            po__qty = promise_qty_array[1]
    ...
    async fetch_pending_free({ request, response, session }) {
        let user_id    = await Redis.get(request.user_id)
...
module.exports = ReceivePoFreeController
