...
class TransferCommunityPalengkeReturn extends Model {
    ...
    */
    ...
    async update_transfer_posted(transfer_id, checked_by, movement_id, movement_no) {
      await Db.table('0_return_transfer')
        .where('id', transfer_id)
        .update({ posted: 1, checked_by, movement_id, movement_no }) 
    }
     ...
     * @param {int} temp_id 
     */
    async add_dispatch_transfer(pos_product, barcode, qty,  temp_id, user_id) {
        ...
        if (temp_id == null) {
            temp_id = await this.add_dispatch_transfer_header(trx, user_id)
        }
        ...
        if (dispatch_transfer_items == 0) {
            await this.add_dispatch_transfer_details(trx, temp_id, productid, barcode, description, uom, qty)
        } else {
    ...
    */
    ...
    async delete_header_items(temp_id, transfer_id) {
        let res = await Db.table('0_return_transfer')
                          .where('id', temp_id)
                          .andWhere('posted', 0)
                          .delete()
        return res
    ...
    async delete_items(id) {
        let res = await Db.table('0_return_transfer_details')
                        .where('id', id)
                        .delete()
        return res
    }
    ...
    async post_receiving(temp_id, user_id, accessId) {
        ...
        try {
          ...
          let checkPlPosted = await srspos.raw(`SELECT TOP 1 * FROM Movements WHERE ReferenceNo = ?`, ['STR-'+temp_id])
            ...
            await srspos.commit()
            await this.update_transfer_posted(temp_id, accessId, movement_id, movement_no)
            return true
        } catch (error) {
            console.log(error)
...
module.exports = new TransferCommunityPalengkeReturn
