...
class TransferDispatch extends Model {
    ...
    async fetch_dispatch_transfer(page, transfer_no) {
        let row = await Db.connection('transfers')
                          .select('a.id', 'a.delivery_date', 'b.name', 'a.auto')
    ...
    */
    ...
    async update_transfer_posted(transfer_id) {
      await Db.table('0_dispatch_transfer')
     ...
     * @param {int} productid 
     */
     async qty_dispatch_item_v1(p_transfer_no, productid, barcode){
      let row = await Db.connection('transfers')
        .select('qty_out')
        .from('0_transfer_details')
        .where('transfer_id', p_transfer_no)
        .andWhere('stock_id', productid)
      return (row[0].qty_out == null) ? 0 : parseFloat(row[0].qty_out)
    }
    /**
     * 
     * @param {int} transfer_no 
    ...
    */
    async fetch_transfer_items_dispatch(transfer_no) {
        let row = await Db.connection('transfers')
                        .select('transfer_id', 'description', 'qty_out', 'uom', 'stock_id_2',  'stock_id', 'barcode')
                        .from('0_transfer_details')
                        .where('transfer_id', transfer_no)  
    ...
    */
    ...
    async post_receiving(temp_id, user_id, checked_by, delivered_by, transfer_id, selling_area_negative=null) {
        ...
        try {
          ...
          let checkPlPosted = await srspos.raw(`SELECT TOP 1 * FROM Movements WHERE ReferenceNo = ?`, ['STO-'+transfer_id])
  ...
  async update_qty(id, qty) {
    return await Db.table('0_dispatch_transfer_details')
      .where('id', id)
