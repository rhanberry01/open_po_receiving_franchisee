...
class TransferCommunityPalengkeDispatch extends Model {
    ...
    async fetch_dispatch_transfer(page, transfer_no) {
        let row = await Db.connection('transfers_palengke')
                          .select('a.id', 'a.delivery_date', Db.raw('"Community Palengke" as name'))
                          .joinRaw('FROM 0_transfer_palengke_header a')
    ...
    async fetch_dispatch_transfer_details(p_transfer_no) {
        let row = await Db.connection('transfers_palengke')
                          .select('a.delivery_date', 'a.date_created', 'a.requested_by', 'b.name as br_code_out', Db.raw('"Community Palengke" as br_code_in'))
    ...
    */
    async fetch_dispatch_header(p_transfer_no) {
        let row = await Db.connection()
                          .select('*')
                          .from('0_dispatch_transfer_palengke')
                          .where('posted', 0)
    ...
    async update_transfer_posted(transfer_id) {
      await Db.table('0_dispatch_transfer_palengke')
    ...
    async qty_dispatch(productid, p_transfer_no, qty) {
        let row = await Db.joinRaw('FROM 0_dispatch_transfer_palengke a INNER JOIN 0_dispatch_transfer_palengke_details b ON a.id = b.temp_receiving_id')
                          .where('transfer_id', p_transfer_no)
    ...
    async add_dispatch_transfer_header(trx, transfer_no, location_to, user_id) {
        ...
        let result = await trx.insert(data)
                              .into('0_dispatch_transfer_palengke')
        return result[0]
    }
    /**
     * @param {int} productid 
    ...
    async fetch_dispatch_transfer_items(productid, barcode, temp_id){
        let row = await Db.from('0_dispatch_transfer_palengke_details')
    ...
    async add_dispatch_transfer_details(trx, temp_id, productid, barcode, description, uom, qty, current_inv){
        ...
        await trx.insert(data)
                 .into('0_dispatch_transfer_palengke_details')       
    }
    /**
     * @param {int} temp_id 
     * @param {int} productid 
    ...
    async update_dispatch_transfer_details(trx, temp_id, productid, barcode, qty) {
        await trx.table('0_dispatch_transfer_palengke_details')
                 .where('temp_receiving_id', temp_id)
                 .andWhere('prod_id', productid)
                 .andWhere('barcode', barcode)
                 .update({ qty }) 
    ...
    async add_dispatch_transfer(pos_product, barcode, qty, location_to, transfer_no, user_id, current_inv, temp_id) {
        ...
        if(gulay != "") {
          if(gulay.levelfield1code == '10061' && gulay.levelfield2code == '0006'){
            await  Db.connection('transfers_palengke')
            .table('0_transfer_palengke_details')
    ...
    async update_last_item_scanned(barcode, prod_id, temp_receiving_id) {
        await Db.connection('receiving_new')
                .table('0_dispatch_transfer_palengke_details')
    ...
    async fetch_last_item_scanned(temp_receiving_id) {
        let row = await Db.connection('receiving_new')
                          ...
                          .from('0_dispatch_transfer_palengke_details')
    ...
    */
    ...
    async counts_items(temp_id, transfer_no) {
        let row = await Db.select('temp_receiving_id')
                          .joinRaw('FROM 0_dispatch_transfer_palengke a INNER JOIN 0_dispatch_transfer_palengke_details b ON a.id = b.temp_receiving_id')
    ...
    async delete_header_items(temp_id, transfer_id) {
        let res = await Db.table('0_dispatch_transfer_palengke')
                          .where('transfer_id', transfer_id)
                          .andWhere('id', temp_id)
                          .delete()
        return res
    ...
    /**
     * delete items 0_dispatch_transfer_palengke_details
     * @param {int} id 
     */
    async delete_items(id) {
        let res = await Db.table('0_dispatch_transfer_palengke_details')
                        .where('id', id)
                        .delete()
        return res
    }
   ...
   async fetch_temporary_items(temp_id) {
    ...
    async update_dispatch_transfer(transfer_id) {
      ...
      return await await Db.connection('receiving_new').table('0_dispatch_transfer_palengke')
                                .where('transfer_id', transfer_id)
                                .update(data)
    }
     ...
     */
    async update_header_transfer(user_id, transfer_id, delivered_by, checked_by) {
        let data = {
            aria_type_out: 70,
            aria_trans_no_out: transfer_id,
            name_out: user_id,
            m_id_out: transfer_id,
            m_no_out: transfer_id,
            m_code_out: 'STO',
            transfer_out_date: moment().format('YYYY-MM-DD HH:mm:ss.SSS'),
            delivered_by: delivered_by,
            checked_by: checked_by,
        ...
        return await  Db.connection('transfers_palengke').table('0_transfer_palengke_header')
                                  .where('id', transfer_id)
                                  .update(data)
    }
    /**
     ...
     * @param {object} row 
     */
    async update_transfer_details(transfer_id, row) {
        let data = {
            actual_qty_out: row.qty
        }

        await Db.connection('transfers_palengke').table('0_transfer_palengke_details')
                           .where('transfer_id', transfer_id)
                           .andWhere('stock_id', row.prod_id)
    ...
    async post_receiving(temp_id, user_id, checked_by, delivered_by, transfer_id, selling_area_negative=null) {
        try {
          ...
          for(const row of temporary_items_list) {
            await this.update_transfer_details(transfer_id, row)
          }

          await this.update_header_transfer(user_id, transfer_id, delivered_by, checked_by)
...
module.exports = new TransferCommunityPalengkeDispatch
