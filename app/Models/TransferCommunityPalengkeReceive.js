...
class TransferCommunityPalengkeReceive extends Model {
    ...
    async fetch_receive_transfer(page, transfer_no) {
        let row = await Db.connection('transfers_palengke')
                          .select('a.id', 'a.delivery_date', Db.raw('"Community Palengke" as branch_in'), 'c.name as branch_out', 'a.transfer_out_date')
                          .joinRaw('FROM 0_transfer_palengke_header a INNER JOIN 0_branches c ON a.br_code_out = c.code')
    ...
    async fetch_transfer_header(p_transfer_no){
        let row = await Db.connection('transfers_palengke')
                          .select('*', 'b.name as br_code_out', Db.raw('"Community Palengke" as br_code_in'), 'b.code as branch_out', 'b.code as branch_in')
                          .joinRaw('FROM 0_transfer_palengke_header a INNER JOIN 0_branches b ON a.br_code_out = b.code')
    ...
    */
    async fetch_receive_header(p_transfer_no) {
        let row = await Db.connection()
                          .select('*')
                          .from('0_receive_transfer_palengke')
                          .where('posted', 0)
    ...
    async qty_receive(productid, p_transfer_no, qty) {
        let row = await Db.joinRaw('FROM 0_receive_transfer_palengke a INNER JOIN 0_receive_transfer_details_palengke b ON a.id = b.temp_receiving_id')
                          .where('transfer_id', p_transfer_no)
    ...
    async add_receive_transfer_header(trx, transfer_no, location_from, user_id) {
        ...
        let result = await trx.insert(data)
                              .into('0_receive_transfer_palengke')
        return result[0]
    }
    /**
     * @param {int} productid 
    ...
    async fetch_receive_transfer_items(productid, barcode, temp_id){
        let row = await Db.from('0_receive_transfer_details_palengke')
    ...
    async add_receive_transfer_details(trx, temp_id, productid, barcode, description, uom, qty, current_inv){
        ...
        await trx.insert(data)
                 .into('0_receive_transfer_details_palengke')       
    }

    async update_transfer_posted(transfer_id) {
      await Db.table('0_receive_transfer_palengke')
    ...
    async update_receive_transfer_details(trx, temp_id, productid, barcode, qty) {
        await trx.table('0_receive_transfer_details_palengke')
                 .where('temp_receiving_id', temp_id)
                 .andWhere('prod_id', productid)
                 .andWhere('barcode', barcode)
                 .update({ qty }) 
    ...
    async update_last_item_scanned(barcode, prod_id, temp_receiving_id) {
        await Db.connection('receiving_new')
                .table('0_receive_transfer_details_palengke')
    ...
    async fetch_last_item_scanned(temp_receiving_id) {
        let row = await Db.connection('receiving_new')
                          ...
                          .from('0_receive_transfer_details_palengke')
    ...
    */
    ...
    async counts_items(temp_id, transfer_no) {
        let row = await Db.select('temp_receiving_id')
                          .joinRaw('FROM 0_receive_transfer_palengke a INNER JOIN 0_receive_transfer_details_palengke b ON a.id = b.temp_receiving_id')
    ...
    async delete_header_items(temp_id, transfer_id) {
        let res = await Db.table('0_receive_transfer_palengke')
                          .where('transfer_id', transfer_id)
                          .andWhere('id', temp_id)
                          .delete()
        return res
    ...
    /**
     * delete items 0_receive_transfer_details_palengke
     * @param {int} id 
     */
    async delete_items(id) {
        let res = await Db.table('0_receive_transfer_details_palengke')
                        .where('id', id)
                        .delete()
        return res
    }
   ...
   async fetch_temporary_product_id_items(temp_id, prod_id) {
    ...
    async add_adjustment_movement_line(srspos, srs_transfers , movement_id, movement_no, transfer_id, user_id, row) {
        try {
            ...
            let old_stock   = parseFloat(row.selling_area_qty) + parseFloat(row.selling_areastock_room) + parseFloat(qty_per_pcs)
    ...
    async fetch_transfer_slip(p_transfer_no, p_date_from, p_date_to) {
        let row = await Db.connection('transfers_palengke')
                          .select('a.*', 'b.name as branch_from', Db.raw('"Community Palengke" as branch_to'))
                          .joinRaw('FROM 0_transfer_palengke_header a INNER JOIN 0_branches b ON a.br_code_out = b.code')
    ...
    async fetch_transfer_slip_header(p_transfer_no) {
        let row = await Db.connection('transfers_palengke')
                          .select('a.date_created', 'a.transfer_out_date', 'a.delivered_by', 'a.checked_by',  'b.name as branch_from', Db.raw('"Community Palengke" as branch_to'), 'a.requested_by')
                          .joinRaw('FROM 0_transfer_palengke_header a INNER JOIN 0_branches b ON a.br_code_out = b.code')
                          .where('a.id', p_transfer_no)
        return row[0]
    }
...
module.exports = new TransferCommunityPalengkeReceive
